...
class Generate_human_error:
    ...
    def generate_variable_human_prediction(self,list_of_std,threshold):
        self.data['c'] = {}
        self.data['test']['c'] = {}
        self.data['y_h'] = {}
        self.data['test']['y_h'] = {}
        ...
        for std in list_of_std:
            h_threshold = np.random.normal(threshold, std, self.data['Y'].shape)
            h = np.zeros(self.data['Y'].shape)
            self.data['y_h'][str(std)] = np.ones(shape=self.data['Y'].shape)
            self.data['c'][str(std)] = np.ones(shape = self.data['Y'].shape)
            self.data['h'][str(std)] = np.ones(shape=self.data['Y'].shape)
            ...
            for idx,label in enumerate(self.data['Y']):
                if label > threshold:
                    ...
                    label = -1
                h[idx] = (self.data['Y'][idx] - h_threshold[idx]) * scale
                # if label == 1:
                #     h[idx] = np.random.uniform(-human_ratio, machine_ratio)
                # if label == -1:
                #     h[idx] = np.random.uniform(-machine_ratio, human_ratio)

                if h[idx]*label<0:
                    self.data['y_h'][str(std)][idx] = -label
                else:
                    self.data['y_h'][str(std)][idx] = label
                self.data['c'][str(std)][idx] = np.maximum(0,1 - (label * h[idx]))
            self.data['h'][str(std)] = h

        for std in list_of_std:
            h_test_threshold = np.random.normal(threshold, std, self.data['test']['Y'].shape)
            h = np.zeros(self.data['test']['Y'].shape)
            self.data['test']['y_h'][str(std)] = np.ones(shape=(self.data['test']['Y'].shape))
            self.data['test']['c'][str(std)] = np.ones(shape=(self.data['test']['Y'].shape))
            self.data['test']['h'][str(std)] = np.ones(shape=(self.data['test']['Y'].shape))
            # h = (self.data['test']['Y']*scale - sample(list,1))
            for idx, label in enumerate(self.data['test']['Y']):
                if label > threshold:
                    label = 1
                else:
                    label = -1
                h[idx] = (self.data['test']['Y'][idx] - h_test_threshold[idx]) * scale
                # if label == 1:
                #     h[idx] = np.random.uniform(-human_ratio, machine_ratio)
                # if label == -1:
                #     h[idx] = np.random.uniform(-machine_ratio, human_ratio)
                ...
                if h[idx] * label < 0:
                    self.data['test']['y_h'][str(std)][idx] = -label
                else:
                    self.data['test']['y_h'][str(std)][idx] = label

                self.data['test']['c'][str(std)][idx] = np.maximum(0, 1 - (label * h[idx]))
            self.data['test']['h'][str(std)] = h
            print self.data['c']
            print self.data['y_h']
            print self.data['test']['c']
            print self.data['test']['y_h']
            print self.data['h']
            print self.data['test']['h']


    def dirichlet_generate_variable_human_prediction(self,list_of_std,threshold):
        self.data['c'] = {}
        self.data['test']['c'] = {}
        self.data['y_h'] = {}
        self.data['test']['y_h'] = {}
        ...
        prob = [[10, 2, 1], [2, 10, 2], [1, 2, 10]]
        label_map = {cats[0]: -1, cats[1]: 0.5, cats[2]: 1}


        for std in list_of_std:
            self.data['y_h'][str(std)] = np.zeros(shape=self.data['Y'].shape)
            self.data['c'][str(std)] = np.zeros(shape=self.data['Y'].shape)
            self.data['h'][str(std)] = np.zeros(shape=self.data['Y'].shape)
            # h = (self.data['Y']*scale - sample(list,1))
            ...
            for idx, label in enumerate(self.data['Y']):
                ...
                error = 0
                for label_idx, probability in enumerate(std_vector):
                    self.data['c'][str(std)][idx] += (probability * (1 - label_map[cats[label_idx]]*thresholded_label))*std
                human_label = np.random.choice(cats, 1, p = std_vector)[0]
                # print human_label
                if human_label>threshold:
                    human_label_thresholded = 1
                else:
                    human_label_thresholded = -1
                self.data['y_h'][str(std)][idx] = human_label_thresholded
                self.data['h'][str(std)][idx] = label_map[human_label]

        for std in list_of_std:
            self.data['test']['y_h'][str(std)] = np.zeros(shape=self.data['test']['Y'].shape)
            self.data['test']['c'][str(std)] = np.zeros(shape=self.data['test']['Y'].shape)
            self.data['test']['h'][str(std)] = np.zeros(shape=self.data['test']['Y'].shape)
            # h = (self.data['Y']*scale - sample(list,1))
            ...
            for idx, label in enumerate(self.data['test']['Y']):
                ...
                error = 0
                for label_idx, probability in enumerate(std_vector):
                    self.data['test']['c'][str(std)][idx] += (probability * (1 - label_map[cats[label_idx]] * thresholded_label)) * std
                human_label = np.random.choice(cats, 1, p=std_vector)[0]
                if human_label > threshold:
                    human_label_thresholded = 1
                else:
                    human_label_thresholded = -1
                self.data['test']['y_h'][str(std)][idx] = human_label_thresholded
                self.data['test']['h'][str(std)][idx] = label_map[human_label]
...
def generate_human_error(file_name_list):
    list_of_option = ['random_noise', 'vary_std_noise', 'norm_rand_noise', \
                      'discrete', 'vary_discrete', 'vary_discrete_old', 'vary_discrete_3', 'dirichlet', 'variable_human']
